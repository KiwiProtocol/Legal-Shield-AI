...
export class AtomaService {
  ...
  async getLegalAdvice(
    ...
    const systemMessage = {
      role: "system",
      content:
        "You are a legal expert AI advisor for LegalShield AI, a decentralized, privacy-first legal service " +
   ...
   */
  private async fallbackLegalAdvice(
    userMessage: string,
    context: Array<{ role: string; content: string }>
  ): Promise<any> {
    ...
    if (courtData && courtData.results && courtData.results.length > 0) {
      ...
      courtData.results.forEach((caseItem: any, index: number) => {
   ...
   */
  private advancedClauseAnalysis(text: string): Record<string, string> {
    const clauseAnalysis: Record<string, string> = {};
    // Define regex patterns for common legal clauses.
    const clauses: { [key: string]: RegExp } = {
   ...
   */
  async getCourtListenerCases(query: string): Promise<any> {
    try {
